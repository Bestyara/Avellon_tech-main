...
Необходимо скачать данный проект, а после из "Проводника" запустить `scripts/app.bat`.
В первый раз установка может занять некоторое время. В последствии запускать
программу можно также через `scripts/app.bat`.

### Быстрый запуск на macOS / Linux

Сделайте скрипты исполняемыми и выполните инициализацию (создаст `venv`, установит зависимости и создаст нужные папки):
...
Для того чтобы открыть проект, нужно выбрать соответствующую папку (она должна быть зарегистрирована как проект в БД).
